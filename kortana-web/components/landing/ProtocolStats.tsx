...
import { getBlockHeight, getTotalTransactions } from '@/lib/rpc';
...
export default function ProtocolStats() {
    ...
    useEffect(() => {
        ...
        // Total transactions — fetched once on mount, then every 30s
        const fetchTx = async () => {
            const tx = await getTotalTransactions();
            if (tx !== "N/A") setTotalTx(tx);
        };
        fetchTx();
        const txInterval = setInterval(fetchTx, 30000);
        ...
        <div className="w-full bg-deep-space/80 backdrop-blur-md border-y border-white/5 py-4 overflow-hidden relative z-30 mb-8">
            <div className="max-w-7xl mx-auto flex items-center justify-between px-4">
                <div className="flex gap-12 animate-marquee whitespace-nowrap">
                    <StatItem label="TOTAL TRANSACTIONS" value={totalTx} />
