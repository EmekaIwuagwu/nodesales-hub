...
import "./interfaces/IKortanaCallee.sol";
...
contract KortanaERC20 {
    string public constant name = "Kortana LP";
...
contract KortanaPair is KortanaERC20 {
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "Kortana: TRANSFER_FAILED");
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "Kortana: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IKortanaFactory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        { // scope for _token{0,1}, avoids stack too deep errors
            ...
            if (data.length > 0) IKortanaCallee(to).kortanaCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
